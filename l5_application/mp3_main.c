...
int current_song_num = 2; // this needs to be set as the same song index in the CLI command
int next_sig = 1;
void read_prev_next_task(void *params) {

  // Receives the song typed by the user (includes signaling for prev, pause, and next buttons)
  char name[32];
  FIL mp3file;
  ...
  while (1) {
    ...
    if (result == FR_OK) { // file does exist!
      ...
      while (file_size > total_read) {
        // sending bytes of mp3 file
        f_read(&mp3file, &bytes_to_send, 512, &bytes_read);
        total_read += bytes_read;

        // Right button: to pause the song

        /*while (playback__is_paused()) {

          vTaskDelay(1);
          //while paused increment current_song_num infinitely
          current_song_num++;
        ...
        */
        ...
        // SEMAPHORE WAY
                ...
                // Left button: to play the next song, by default goes in here
                if (xSemaphoreTake(Next_Signal, portMAX_DELAY)) {
                  fprintf(stderr, "next song to be played\n");
                  current_song_num++;
                ...
                // Middle button: to play the prev song
                if (xSemaphoreTake(Prev_Signal, portMAX_DELAY)) {
                  fprintf(stderr, "prev song to be played\n");
                  current_song_num--;
                  // name = tracklist[current_song_num];
                  // xQueueSend(Song_Q, &tracklist[current_song_num], portMAX_DELAY);
                  // break;
        ...
        */
        // sending data of selected mp3 song to decoder
